...
model = load_model('L:/Codefield/Python/face_recogniazation/model/fas.h5')
...
class OpcvCapture(threading.Thread):
    def __init__(self, win_name, cam_name):
        ...
        x = input('请选择方法,1=dlib+眨眼,2=dlib+静默,3=opencv+欧氏距离,4=opencv+l2距离\n')
        if x == '1':
    ...
    def photo2(self):
        name=[]
        if os.path.isfile('./csvdata/figures1.csv'):
            print('csv file exist, start to read csv')
            with open('./csvdata/figures1.csv', newline='',encoding='utf-8') as csvfile:
                d = np.loadtxt(csvfile , delimiter=",", dtype=float)
                #print(d)
            with open('./csvdata/name.csv', newline='', encoding='utf-8') as f:
                for row in f:
            ...
            for i in path:
                img = cv2.imdecode(np.fromfile(os.path.join(facepath, i), dtype=np.uint8), cv2.IMREAD_COLOR)
                face_descriptor = recognizer.feature(img)
                d.append(face_descriptor[0])

            print('Start to write csv, please wait...')
            #d.reshape(len(d),128)
            #print(d)
            with open('./csvdata/figures1.csv', 'w', newline='',encoding='utf-8') as csvfile:
                np.savetxt(csvfile, d, delimiter=',', fmt='%f')
            with open('./csvdata/name.csv', newline='', encoding='utf-8') as f:
                for row in f:
                    name.append(row[:-2])
    ...
    def method1(self,gray,face,list,name):
    ...
    def method2(self, gray, face, list,name):
        landmarks = predictor(gray,face)
        self.eyejudge(landmarks)
        video = recognizer.feature(gray)
        q=[]
        for i in list:
            i = np.array(i).reshape(1, -1)
            i = i.astype(np.float32)
            #np.set_printoptions(suppress=False, formatter={'float': '{:0.6e}'.format})
            q.append(recognizer.match(video,i, 0))
        max_value = max(q) # 求列表最小值
        max_idx = q.index(max_value)
        return name[max_idx]
    
    def method3(self, gray,face, list,name):
        video = recognizer.feature(gray)
        q=[]
        for i in list:
            i = np.array(i).reshape(1, -1)
            i = i.astype(np.float32)
            #np.set_printoptions(suppress=False, formatter={'float': '{:0.6e}'.format})
            q.append(recognizer.match(i, video, 1))
        min_value = min(q) # 求列表最小值
        min_idx = q.index(min_value)
        return name[min_idx]
    ...
    def run(self):
        ...
        while (True):
            ...
            #k = cv2.waitKey(1)
            #gray = cv2.cvtColor(src=frame, code=cv2.COLOR_BGR2RGB)
            ...
            if len(faces) != 0:
                for face in faces:
                    ...
                    cv2.putText(frame, str(score), (5,50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
                    cv2.putText(frame, "Blinks:{0}".format(self.blink_counter), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
